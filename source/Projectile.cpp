...
Projectile::Projectile(SDL_Renderer *renderer, Vector2D setPos, Vector2D setDirectionNormal,
                       int damage, bool splash,int chain,float slow,float distMax) :
        pos(setPos), directionNormal(setDirectionNormal), damage(damage), splash(splash),chain(chain),slow(slow),distanceTraveledMax(distMax) {
        ...
        for (int count = 0; count < listUnits.size() && !collisionOccurred; count++) {
            ...
            if (unitSelected != nullptr && unitSelected->checkOverlap(pos, size)) {
                if (splash) {
                    ...
                    float splashSize = 1.5f;
                    int damaged = 0;
                    for (int count = 0; count < listUnits.size() && !collisionOccurred && damaged <= chain; count++) {
                        auto &unitSelected = listUnits[count];
                        if (unitSelected != nullptr && unitSelected->checkOverlap(pos, splashSize)) {
                            unitSelected->removeHealth( damage * pow(0.6,damaged), slow);
                            damaged++;
                        }
                    }
                }
